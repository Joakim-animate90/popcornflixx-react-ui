...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log('Here is our popularMoviesResponse:', popularMoviesResponse);
        // Extract the results array from the API response
        const moviesData = popularMoviesResponse?.results || [];
        setMovieArray(Array.isArray(moviesData) ? moviesData : []);
      } catch (error) {
    ...
    const fetchActivity = async () => {
      try {
        if (user && user.watched) {
          ...
          setActivity(Array.isArray(updatedActivty) ? updatedActivty.slice(0, 5) : []);
        }
      } catch (error) {
        toast.error(error.message, toastConfig);
        console.error(error);
  ...
  return (
    <div className='dashboard'>
      ...
      {activity.length > 0 && (
        <>
          ...
          <div className='activity-container'>
            {Array.isArray(activity) && activity.map((movie, index) => (
              <MovieCard
                key={index}
                movie={movie}
              />
      ...
      <p className='section-heading'>popular on popcornflixx</p>
      <div className='popular-container'>
        {Array.isArray(movieArray) && movieArray.map((movie, index) => (
          <MovieCard
            key={index}
            movie={movie}
          />
